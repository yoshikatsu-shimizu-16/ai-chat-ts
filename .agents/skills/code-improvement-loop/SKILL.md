...
1. Read `.improvement-loop/active-rules.md` when the SessionStart hook has not already supplied it.
...
```bash
node scripts/improvement-loop/cli.mjs record \
...
```bash
node scripts/improvement-loop/cli.mjs status
node scripts/improvement-loop/cli.mjs verify --session-id "<session_id>" --turn-id "<turn_id>" --status passed --command "pnpm test"
node scripts/improvement-loop/cli.mjs promote
```
