...
export default function Chat() {
  ...
  const [chatStreamingAssistantReply, setChatStreamingAssistantReply] =
    useState("");
  // AI回答の生成中かどうかを管理する。
  ...
  useEffect(() => {
    ...
    const storedChatHistoryJson = sessionStorage.getItem(
  ...
  /**
   * フォーム入力をチャットAPIへ送り、ストリーミング応答を表示履歴へ反映する。
   * @param event チャットフォームの送信イベント
   * @returns チャット応答処理の完了を示すPromise
   */
  async function handleChatSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const chatResponse = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ message: chatUserMessage.content }),
      ...
      while (true) {
        ...
        if (done) break;
        completeChatAssistantReply += chatResponseDecoder.decode(value, {
          stream: true,
        });
  ...
  const hasChatMessages =
    chatHistory.length > 0 || chatStreamingAssistantReply.length > 0;
    ...
    <section className="chat">
      ...
      <form onSubmit={handleChatSubmit}>
        ...
        <div className="actions">
          <button
            type="submit"
            disabled={isChatResponseLoading || !chatInputText.trim()}
          >
            送信
          </button>
          {isChatResponseLoading && (
            <button
              type="button"
              onClick={() => chatResponseAbortControllerRef.current?.abort()}
            >
              停止
            </button>
          )}
